...
export default class Nexus extends LivingEntity {
    ...
    public spawnBase: TeamBase;
    public mineBase: TeamBase;

    private lastNotificationTick = 0;
    
    public constructor(game: GameServer, x: number, y: number, team: TeamEntity, config: NexusConfig, nexusBase: TeamBase, spawnBase: TeamBase, mineBase: TeamBase) {
        super(game);
    ...
    public notifyDamage() {
        if(this.game.arena.state !== ArenaState.OPEN) return;
        if(this.lastNotificationTick + tps * 5 > this.game.tick) return;
        ...
        for(const client of this.game.clients) {
            ...
            client.notify(notification, ColorsHexCode[Color.TeamRed], 5000, 'nexus_attack');
        }
    ...
    public sacrifice(client: Client) {
        ...
        if((this.game.arena as EventArena).invincibilityTimeLeft) return client.notify("Sacificing disable in this phase.", 0xFFA500, 2000, 'cant_claim_info');
    ...
    public onDeath(killer: LivingEntity): void {
        ...
        this.mineBase.setPainful(false);
        this.base.styleData.color = Color.Neutral;
        this.base.styleData.color = Color.Neutral;
        this.base.styleData.color = Color.Neutral;
        ...
        this.game.broadcast()
            ...
            .u32(killer.styleData ? ColorsHexCode[killer.styleData.color] : 0x000000)
            .float(15000)
            .stringNT("").send();
    }
    ...
    public tick(tick: number) {
        if((this.game.arena as EventArena).invincibilityTimeLeft > 0) {
        ...
        if((this.game.arena as EventArena).invincibilityTimeLeft > 0) return;
